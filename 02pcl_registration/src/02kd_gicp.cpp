...
{ 
    ...
    gicp.setSearchMethodSource(tree_source);
    gicp.setSearchMethodTarget(tree_target);

    // gicp核心部分
    gicp.setInputSource(cloud_source);         // 源点云
    gicp.setInputTarget(cloud_target);         // 目标点云
    gicp.setTransformationEpsilon(1e-10);       // 最小转换差异  
    gicp.setMaxCorrespondenceDistance(10.0);    // 最大对应点距离
    gicp.setEuclideanFitnessEpsilon(0.001);     // 收敛条件是均方差的和小于阈值，停止迭代
    ...
    auto gicp_cloud = std::make_shared<pcl::PointCloud<pcl::PointXYZ>>();
    gicp.align(*gicp_cloud, init_transform);
    ...
    if(gicp.hasConverged()){
        ...
        std::cout << "Align score is: "  << gicp.getFitnessScore() << std::endl;
